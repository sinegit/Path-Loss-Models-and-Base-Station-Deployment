...
Area=len*bred;
digits(8);
Data = readtable('Water_Data2.csv','ReadRowNames',true,'Format','%u%f%f');
S = size(Data,1); %Number of sensors
Sensor = zeros(S,2);
disp('Number of Sensors deployed:')
disp(S)
Data_BS = readtable('BS_Locations.txt','ReadRowNames',true,'Format','%u%f%f');
B = size(Data_BS,1); %Number of BSs
BaseStation = zeros(B,2);
rad = 0.75; %Coverage radius of BS in km
